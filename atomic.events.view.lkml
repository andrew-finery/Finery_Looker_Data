view: atomic_events {
  ...
  dimension: page_type {
    ...
    case: {
      when: {
        sql: ${page_urlpath_adjusted_for_country} in ('/', '//') ;;
        label: "Homepage"
      }

      when: {
        sql: ${page_url_path} like '%/t/%' ;;
        label: "Product Listing Page"
      }

      when: {
        sql: ${page_url_path} like '%/products/%' ;;
        label: "Product Detail Page"
      }

      when: {
        sql: ${page_urlpath_adjusted_for_country} = '/chapters' ;;
        label: "Chapters Home"
      }

      when: {
        sql: ${page_url_path} like '%/chapters/%' ;;
        label: "Editorial"
      }

      when: {
        sql: ${page_urlpath_adjusted_for_country} = '/cart' ;;
        label: "Cart"
      }

      when: {
        sql: ${page_urlpath_adjusted_for_country} = '/checkout' ;;
        label: "Checkout"
      }

      when: {
        sql: ${page_url_path} like '/orders/%' ;;
  ...
  measure: count_total_opens {
    ...
    filters: {
      field: structured_event
      value: "open"
    }

    filters: {
      field: structured_event_category
      value: "email"
    }
