view: adwords_criteria_performance {
  ...
  dimension: Brand_Non_Brand {
    sql: case when ${TABLE}.campaign like '%brand%' or ${TABLE}.campaign like '%Brand%' then 'Brand' else 'Non Brand' end
      ;;
  }

  dimension: Country {
    sql: case when ${TABLE}.accountdescriptivename like 'uk_%' then 'UK'
  ...
  dimension: audience_segment {
    sql: case when ${TABLE}.campaign like '%acquisition%' then 'acquisition'
